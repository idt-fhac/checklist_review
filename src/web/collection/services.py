...
def iterate_collection(folder_path: str, collection_name_override: str | None = None) -> Generator[ProgressEvent | ResultEvent, None, None]:
    ...
    if not provider_config and not is_rule_based_pdf_metadata_extraction():
        raise PaperProcessingError("No LLM provider configured. Please add an LLM provider in settings or use rule-based PDF metadata extraction.")
    
    # Process all PDFs using the new method
    for index, pdf in enumerate(pdf_files, start=1):
        yield ProgressEvent(type="progress", current=index, total=total, filename=pdf.name)
        ...
        extracted_json_path = meta_dir / f"{pdf.stem}.json" if meta_dir.exists() else collection_dir / "source_extracted" / f"{pdf.stem}.json"
    ...
    collection_name = collection_name_override or derive_collection_name(str(collection_dir))
    try:
