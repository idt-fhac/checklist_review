...
def build_prompt_with_pages(criterion_text: str, paper_content: str, artifact_pages: Optional[List[Dict[str, Any]]]) -> str:
    ...
    if rag_separator in paper_content:
        ...
        for chunk in chunks:
            ...
            page_match = re.search(r'\n\n?PAGE NUMBER\s+(\d+|Unknown)\s*$', chunk, re.IGNORECASE | re.MULTILINE)
            if page_match:
                page_num_str = page_match.group(1)
                content = re.sub(r'\n\n?PAGE NUMBER\s+\d+|Unknown\s*$', '', chunk, flags=re.IGNORECASE | re.MULTILINE).strip()
                page_num = page_num_str if page_num_str != "Unknown" else "?"
            else:
                page_match_anywhere = re.search(r'PAGE NUMBER\s+(\d+|Unknown)', chunk, re.IGNORECASE)
                if page_match_anywhere:
                    page_num_str = page_match_anywhere.group(1)
                    page_num = page_num_str if page_num_str != "Unknown" else "?"
                    content = re.sub(r'\n\n?PAGE NUMBER\s+\d+|Unknown\s*', '', chunk, flags=re.IGNORECASE).strip()
                else:
                    page_num = "?"
                    content = chunk.strip()
