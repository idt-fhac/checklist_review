    ...
    for st in supporting_texts:
        if isinstance(st, dict):
            result["supporting_texts"].append({
                ...
                "short_explanation": str(st.get("short_explanation", "No explanation provided"))
            })
    if not result["supporting_texts"]:
        result["supporting_texts"] = [{"page_number": -1, "text_crop": "No supporting text extracted from fallback parsing", "short_explanation": "Fallback parsing did not extract any supporting text"}]
    return result
...
def extract_from_text_fallback(response_text: str) -> Dict[str, Any]:
    ...
    supporting_texts = []
    st_match = re.search(r'"supporting_texts"\s*:\s*\[(.*?)\]', response_text, re.DOTALL | re.IGNORECASE)
    
    if st_match:
        st_content = st_match.group(1)
        st_objects = re.finditer(r'\{[^{}]*(?:"page_number"[^{}]*"text_crop"|"text_crop"[^{}]*"page_number")[^{}]*\}', st_content, re.DOTALL)
        for obj_match in st_objects:
            try:
                ...
                supporting_texts.append({
                    ...
                    "short_explanation": str(st_obj.get("short_explanation", "Extracted from fallback parsing"))
                })
    ...
    if not supporting_texts:
        supporting_texts = [{"page_number": -1, "text_crop": response_text[:500], "short_explanation": "Full response text used as supporting text"}]
    
    return {"answer": answer, "supporting_texts": supporting_texts}


def extract_from_text_only(agent, prompt: str, provider_config: Dict[str, Any],
    ...
    supporting_texts = []
    page_patterns = [r'page\s+(\d+)', r'p\.\s*(\d+)', r'pg\.\s*(\d+)', r'on\s+page\s+(\d+)']
    ...
    if not supporting_texts:
        ...
        supporting_texts = [{"page_number": -1, "text_crop": " ".join(relevant_sentences) if relevant_sentences else response_text[:500], "short_explanation": "Analysis extracted from full response"}]
    
    return {"answer": answer, "supporting_texts": supporting_texts}


def fallback_structured_output(agent, prompt: str, provider_config: Dict[str, Any],
                              token_usage_accumulator: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def parse_batch_json_from_text(response_text: str, questions: List[Dict[str, Any]]) -> Optional[List[Dict[str, Any]]]:
    try:
...
def fallback_batch_structured_output(agent, prompt: str, provider_config: Dict[str, Any],
                                     questions: List[Dict[str, Any]],
                                     token_usage_accumulator: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    example_answers = []
    for q in questions:
