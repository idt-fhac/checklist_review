...
from src.review_workflow.components.evaluators.criterion_evaluator.agent_creator import (
    create_review_agent,
)
...
from src.review_workflow.components.evaluators.criterion_evaluator.models import (
    BatchReviewResponse,
    ReviewResponse,
)
...
from src.review_workflow.components.evaluators.criterion_evaluator.rag_context import (
    prepare_rag_context,
)
...
class CriterionEvaluator(BaseComponent):
    def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not criterion:
            return self._log_error(
                "'criterion' or 'criteria' is required", log_callback
            )
    ...
    def _apply_section_mapping(
        ...
        mapped = section_mapping.get(str(criterion_id)) or section_mapping.get(
            criterion_id
        )
        if not mapped:
            return md_content, artifact_pages
    ...
    def _execute_single(
        ...
        artifact_json_path, artifact_md_path = get_artifact_file_paths(
            collection_name,
            pipeline_name,
            artifact_name,
            criteria_set_name,
        ...
        if not artifact_json_path.exists():
            return self._log_error(
                f"Paper metadata JSON not found: {artifact_json_path}", log_callback
            )
        ...
        if method == "Direct File Upload":
            return self._log_error(
                "Direct File Upload method is not yet implemented. Please reprocess the paper with 'Extracted Content' method.",
                log_callback,
            )

        if not artifact_md_path.exists():
            return self._log_error(
                f"Paper markdown file not found. Method was: '{method}'. Please reprocess the paper with 'Extracted Content' method.",
                log_callback,
            )
        ...
        if not md_content.strip():
            return self._log_error(
                f"Paper content is empty. Method: {method}", log_callback
            )
        ...
        agent = create_review_agent(
            provider_config, runtime_config, artifact_pages, context=context
        )
        result = self._get_review_result(
            agent, prompt, provider_config, log_callback, token_usage_accumulator
        )
        result["criterion_id"] = criterion_id
        result["criterion_text"] = criterion.get("description") or criterion.get(
            "text", ""
        )
        if persona_id:
            result["persona_id"] = persona_id
        result["supporting_texts"] = enhance_supporting_texts_with_highlighting(
            result.get("supporting_texts", []), artifact_pages
        )
    ...
    def _execute_batch(
        ...
        evaluations_file = get_evaluations_file_path(
            collection_name,
            pipeline_name,
            artifact_name,
            criteria_set_name,
        ...
        if not questions_to_answer:
            if log_callback:
                log_callback(
                    f"All {len(criteria)} questions already have answers", "info"
                )
        ...
        if self.config.get("use_rag", False) and log_callback:
            log_callback(
                "Warning: RAG is not supported in batch mode. Using full paper content instead.",
                "warning",
            )
        ...
        agent = create_review_agent(
            provider_config, self.config, artifact_pages, context=context
        )
        batch_result = self._get_batch_review_result(
            agent,
            prompt,
            provider_config,
            questions_to_answer,
        ...
        for q in questions_to_answer:
            ...
            if not answer_data:
                if log_callback:
                    log_callback(
                        f"No answer returned for question id {criterion_id}; using error placeholder",
                        "warning",
                    )
                answer_data = self._error_response(
                    "Batch did not return an answer for this question."
                )
    ...
    def _error_response(self, reason: str):
        return {
            "answer": False,
            "supporting_texts": [
                {"page_number": -1, "text_crop": reason, "short_explanation": reason}
            ],
            "error": reason,
        }
    ...
    def _ensure_batch_one_per_question(
        ...
        for q in questions:
            ...
            if r is not None:
                ...
                if log_callback:
                    log_callback(
                        f"Filling missing answer for question id {qid}", "warning"
                    )
                ordered.append(
                    {
                        "criterion_id": qid,
                        ...
                        "supporting_texts": [
                            {
                                "page_number": -1,
                                "text_crop": default_error,
                                "short_explanation": default_error,
                            }
                        ],
                    }
                )
    ...
    def _get_review_result(
        ...
        if self.config.get("use_json_fallback", False):
            return fallback_structured_output(
                agent, prompt, provider_config, token_usage_accumulator
            )
        ...
        try:
            ...
            if (
                "structured output" not in error_msg
                and "structured_output" not in error_msg
            ):
                raise

            if log_callback:
                log_callback(
                    "Structured output failed, attempting fallback parsing", "warning"
                )

            try:
                result = fallback_structured_output(
                    agent, prompt, provider_config, token_usage_accumulator
                )
                if log_callback:
                    log_callback(
                        "Successfully parsed response using fallback method", "info"
                    )
                return result
            except Exception:
                if log_callback:
                    log_callback(
                        "Fallback parsing failed, trying simplified extraction...",
                        "warning",
                    )
                try:
                    result = extract_from_text_only(
                        agent, prompt, provider_config, token_usage_accumulator
                    )
                    if log_callback:
                        log_callback(
                            "Successfully extracted response using text-only method",
                            "info",
                        )
                    return result
    ...
    def _get_batch_review_result(
        ...
        if self.config.get("use_json_fallback", False):
            return fallback_batch_structured_output(
                agent, prompt, provider_config, questions, token_usage_accumulator
            )
        ...
        try:
            ...
            if missing_ids and log_callback:
                log_callback(
                    f"Warning: {len(missing_ids)} questions not answered in batch response. Attempting fallback.",
                    "warning",
                )
                for missing_id in missing_ids:
                    question = next(
                        (q for q in questions if str(q.get("id", "")) == missing_id),
                        None,
                    )
                    if question:
                        ...
                        try:
                            single_response = agent(
                                single_prompt, structured_output_model=ReviewResponse
                            )
                            if token_usage_accumulator is not None:
                                token_usage_add(
                                    token_usage_accumulator, single_response, agent
                                )
                            single_result = (
                                single_response.structured_output.model_dump()
                            )
                            result_list.append(
                                {
                                    "criterion_id": raw_id,
                                    "answer": single_result.get("answer", False),
                                    "supporting_texts": single_result.get(
                                        "supporting_texts", []
                                    ),
                                }
                            )
                            ...
                            if log_callback:
                                log_callback(
                                    f"Failed to answer question {raw_id} in fallback: {str(e)}",
                                    "error",
                                )
                            result_list.append(
                                {
                                    ...
                                    "supporting_texts": [
                                        {
                                            "page_number": -1,
                                            "text_crop": f"Failed to answer: {str(e)}",
                                            "short_explanation": "Error during batch processing",
                                        }
                                    ],
                                }
                            )

            return self._ensure_batch_one_per_question(
                result_list, questions, log_callback
            )
            ...
            if log_callback:
                log_callback(
                    "Structured output failed for batch, attempting fallback parsing",
                    "warning",
                )

            try:
                result = fallback_batch_structured_output(
                    agent, prompt, provider_config, questions, token_usage_accumulator
                )
                if log_callback:
                    log_callback(
                        "Successfully parsed batch response using fallback method",
                        "info",
                    )
                return self._ensure_batch_one_per_question(
                    result, questions, log_callback
                )
            except Exception:
                if log_callback:
                    log_callback(
                ...
                try:
                    result = extract_batch_from_text_only(
                        agent,
                        prompt,
                        provider_config,
                        questions,
                    ...
                    if log_callback:
                        log_callback(
                            "Successfully extracted batch response using text-only method",
                            "info",
                        )
                    return self._ensure_batch_one_per_question(
                        result, questions, log_callback
                    ...
                    if log_callback:
                        log_callback(error_detail, "error")
                    return self._ensure_batch_one_per_question(
                        [], questions, log_callback, default_error=error_detail
                    )
