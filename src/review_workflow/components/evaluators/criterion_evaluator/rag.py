...
def _chunk_by_page(text: str, artifact_pages: Optional[List[Dict[str, Any]]] = None) -> List[Dict[str, Any]]:
    ...
    if artifact_pages:
        for page_info in artifact_pages:
            ...
            if start_pos >= 0:
                prev_markers = list(re.finditer(r"---\s+end\s+of\s+page=\d+\s+---", text[:start_pos]))
...
class RAG:
    def __init__(self, embedding_provider_id: Optional[str] = None):
        if not embedding_provider_id:
            raise ValueError("RAG requires an embedding provider; select one in the Question Reviewer node.")
...
def format_chunks_for_prompt(chunks: List[Dict[str, Any]]) -> str:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        formatted_parts.append(f"PAGE NUMBER {page_number if page_number > 0 else 'Unknown'}")
        if i < len(chunks):
