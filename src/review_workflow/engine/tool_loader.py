...
REVIEW_COMPONENTS_DIR = (
    Path(__file__).resolve().parent.parent / "components" / "evaluators"
)
...
def discover_review_tools() -> Dict[str, Dict[str, Any]]:
    ...
    if not REVIEW_COMPONENTS_DIR.exists():
        logger.warning(
            f"Review components directory not found: {REVIEW_COMPONENTS_DIR}"
        )
        return tools
...
def load_tool_class(
    tool_id: str, tools_registry: Optional[Dict[str, Dict[str, Any]]] = None
    ...
    try:
        ...
        class_name = (
            tool_info["component_dir"].name.replace("_", " ").title().replace(" ", "")
        )
        ...
        for attr_name in dir(module):
            ...
            try:
                ...
                if (
                    ...
                    and attr_name.lower() == class_name.lower()
                ):
            ...
            for attr_name in dir(module):
                ...
                try:
                    ...
                    if (
                        ...
                        component_class = attr
                        logger.info(
                            f"Found component class {attr_name} for tool {tool_id}"
                        )
                        break
                except (TypeError, AttributeError):
                    continue
        ...
        if component_class is None:
            logger.error(
                f"Could not find component class for tool {tool_id} in {module_path}"
            )
            return None
    ...
    try:
        ...
        if context and context.get("log_callback"):
            context["log_callback"](
                f"{error_msg}. Check server logs for details.", "error"
            )
        return None
